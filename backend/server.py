...
CANCELLATION_POLICIES = {
    "flexible": {
        "label": "Flexible",
        "description": "Full refund if cancelled 24+ hours before pickup. No refund inside 24 hours.",
        "tiers": [(24, 1.0)],  # (hours before start, refund fraction) — first matching tier wins
    },
    "moderate": {
        "label": "Moderate",
        "description": "Full refund 5+ days before pickup, 50% refund 1-5 days before, no refund inside 24 hours.",
        "tiers": [(24 * 5, 1.0), (24, 0.5)],
    },
    "strict": {
        "label": "Strict",
        "description": "Full refund 14+ days before pickup, 50% refund 7-14 days before, no refund inside 7 days.",
...
def compute_refund_fraction(policy_key: str, start_date: str, has_trip_started: bool) -> float:
    ...
    if has_trip_started:
        return 0.0  # already picked up — cancellation policy no longer applies

    policy = CANCELLATION_POLICIES.get(policy_key, CANCELLATION_POLICIES["moderate"])

    hours_until_start = None
    if start_date and start_date != "TBD":
        try:
            parsed = dateparser.parse(start_date, fuzzy=True, default=datetime.now(timezone.utc))
            if parsed.tzinfo is None:
                parsed = parsed.replace(tzinfo=timezone.utc)
            hours_until_start = (parsed - datetime.now(timezone.utc)).total_seconds() / 3600
        except Exception:
            hours_until_start = None

    if hours_until_start is None:
        # No parseable date on the booking — can't measure "how close to the
        # trip," so default to a full refund as long as the trip hasn't
        # started yet (fair to the renter given the ambiguity).
        return 1.0

    for threshold_hours, fraction in policy["tiers"]:
        if hours_until_start >= threshold_hours:
            return fraction
    return 0.0
...
async def set_booking_status(bid: str, data: BookingStatusIn, user: dict = Depends(get_current_user)):
    ...
    refund_info = None
    if target == "cancelled" and item.get("paid") and item.get("payment_intent_id"):
        listing = await db.listings.find_one({"id": item["listing_id"]}, {"_id": 0, "cancellation_policy": 1})
        policy_key = (listing or {}).get("cancellation_policy", "moderate")
        fraction = compute_refund_fraction(policy_key, item.get("start_date", "TBD"), has_trip_started=(current == "active"))
        if fraction > 0:
            refund_cents = int(round(item["subtotal"] * fraction * 100))
            ...
            try:
                await run_in_threadpool(_refund)
                refund_info = {"refund_pct": round(fraction * 100), "refund_amount": round(refund_cents / 100, 2)}
            except Exception as e:
                logger.error(f"stripe refund failed: {e}")
                raise HTTPException(status_code=502, detail="Cancellation recorded, but the automatic refund failed — contact support.")
        else:
            refund_info = {"refund_pct": 0, "refund_amount": 0}

    update = {"status": target}
    if refund_info is not None:
        update["refund_pct"] = refund_info["refund_pct"]
        update["refund_amount"] = refund_info["refund_amount"]
        update["refunded_at"] = now_iso()
    ...
    if target == "approved":
        ...
        if refund_info:
            msg += f" ${refund_info['refund_amount']} ({refund_info['refund_pct']}%) refunded." if refund_info["refund_amount"] else " No refund applies per the cancellation policy."
        await notify(other, "Booking cancelled", msg, "booking_cancelled", {"booking_id": bid})
...
async def cancellation_policies():
    return {k: {"label": v["label"], "description": v["description"]} for k, v in CANCELLATION_POLICIES.items()}
